...
def save(ind):
    ...
    incre += 1
    value1[ind] = incre
    total = int(value1[len(value1) - 1])
    total += 1
    value1[len(value1) - 1] = total
    data1 = [label1, value1]
...
def update(txt, ind):
    ...
    switch.config(text=total)
    press[len(press) - 1].config(text=total)
...
if __name__ == '__main__':
    ...
    frame1 = Frame(root1, bg="grey", pady=15)
    frame1.grid(row=0, column=0)

    name = Label(frame1, text='Total Presses', fg="white", bg="grey", font=("Helvetica", 14))
    name.pack(side="left")

    switch = Label(frame1, text='', fg="white", bg="grey", font=("Helvetica", 14))
    switch.pack(padx=40, side="left")

    value = StringVar()
    entry_box = Entry(frame1, textvariable=value, takefocus=0, width=10, highlightthickness=2, font=("Helvetica", 16))
    ...
    b_name.grid(row=1, column=0, padx=50)
    press = Label(h_frame, text="Presses", bg="grey", fg="white", font=('Helvetica', 14))
    press.grid(row=1, column=1, padx=50)
    wear = Label(h_frame, text="Wear %", bg="grey", fg="white", font=('Helvetica', 14))
    wear.grid(row=1, column=2, padx=70)
    ...
    canvas = Canvas(frame, bd=0, height=500, width=673, yscrollcommand=yscrollbar.set)
